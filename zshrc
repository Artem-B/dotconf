...
#
# $Id: .zshrc,v 1.228 2003/03/18 21:54:17 adams Exp $
#
...
if (( $#_find_promptinit >= 1 )) && [[ -r $_find_promptinit[1] ]]; then
  ...
  if [[ $TERM == tgtelnet ]]; then
    ...
    PROMPT='%n@%m%(!.#.$) '
    RPROMPT=' %~ %t'
    if [[ -n $TERM  && `echo $TERM | egrep '(rxvt|xterm)'` != "" ]]; then
	# Set prompt to change titlebar
	RPROMPT="$RPROMPT%{]2;%n@%m:%~]1;%n@%m:%~%}"
    fi
#    PS1='%n@%m %B%3~%b %# '
fi
...
if [[ "$ZSH_VERSION_TYPE" == 'new__DISABLED' ]]; then
  : ${(A)_etc_hosts:=${(s: :)${(ps:\t:)${${(f)~~"$(<$HOME/.hosts)"}%%\#*}##[:blank:]#[^[:blank:]]#}}}
...
if [[ "${TERM}" == ([Ex]term*|dtterm|rxvt*|screen*) ]]; then
  unset TMOUT
fi
